    ...
    public List<Map<String, Object>> forListMap(Document document, int top) {
        ...
        if (ObjectUtils.isEmpty(document)) {
            ...
            for (int i = 0; i < (elements.size() > top ? top : elements.size()); i++) {
                ...
                result.put("ip", element.getElementsByTag("tr").get(0).getElementsByTag("td").get(1).text());
                result.put("port", Integer.valueOf(element.getElementsByTag("tr").get(0).getElementsByTag("td").get(2).text()).intValue());
                ...
                result.put("geo", element.getElementsByTag("tr").get(0).getElementsByTag("td").get(6).text());
                results.add(result);
            }
        }
        return results;
